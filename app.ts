...
const totalPrice = ({ price, discount, isInstallment, months }: Data): number => {
...
const normalizeData = (unnormalizedData: Article[]): NormalizedData => {
    let result: Partial<NormalizedData>= {}
    let byId;
    let idArr: string[] = []
...
async function getData(url: string): Promise<CommentItem[]> {
